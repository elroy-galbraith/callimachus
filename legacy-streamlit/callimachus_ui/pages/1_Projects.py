...
from callimachus.project import create_from_template, list_projects
...
st.set_page_config(page_title="Projects · Callimachus", layout="wide")
project_chip()
...
# Available templates: every callimachus/pack/builtin/<name>/ that has a pack.yaml.
templates: list[str] = sorted(
    p.name for p in BUILTIN_DIR.iterdir() if (p / "pack.yaml").exists()
) if BUILTIN_DIR.exists() else []

if not templates:
    st.error("No built-in pack templates found under callimachus/pack/builtin/.")
else:
